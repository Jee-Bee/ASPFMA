...
location (two samples either side and the sample at p itself) and compare it with the threshold. During
the computation of standard deviation divide by the number of samples (=5) and not 4, i.e. without
the Bessel's correction. Based on the comparison, return a boolean variable selectFlag, which is True if
the standard deviation is less than the threshold (and hence the phase is flat), else False (phase is
not flat). We will use a small phase deviation threshold of 0.01 radian. In short, selectFlatPhasePeak()
that returns True if the standard deviation of five samples of the phase spectrum pX around the input
index p is less than the given threshold, else False.
...
def selectFlatPhasePeak(pX, p, phaseDevThres):
    ...
    """
    # see: sms-tools/lectures/05-Sinusoidal-model/plots-code/sineModelAnal-bendir-phase.py

    # compute std deviation based on 5 peaks/ points?
    std_pX = np.std(pX[p-2:p+3])  # p-2 = 1 AND p+2 = 5 but i need incl 5 so p + 3
    selectFlag = std_pX < phaseDevThres
    print(std_pX, phaseDevThres, std_pX<phaseDevThres)
    return(selectFlag)


### Go through the code below and understand it, but do not modify anything ###
def sineModelAnalEnhanced(inputFile= '../sms-tools/sounds/sines-440-602-transient.wav'):
    """
    Input:
...
p = 3  #, and
phaseDevThres = 0.25  # , the function
selectFlag = selectFlatPhasePeak(pX, p, phaseDevThres) #returns selectFlag = True.
print(selectFlag, True)

#Test case 2:
# With
pX = np.array([1.0, 1.2, 1.3, 1.4, 0.9, 0.8, 0.7, 0.6, 0.7, 0.8])  # ,
p = 3  # , and
phaseDevThres = 0.1  # , the function
selectFlag = selectFlatPhasePeak(pX, p, phaseDevThres)  # returns selectFlag = False.
print(selectFlag, False)
...
0.87, 0.51, 0.67])  # ,
p = 17  # , and
phaseDevThres = 0.01  # , the function
selectFlag = selectFlatPhasePeak(pX, p, phaseDevThres)  # returns selectFlag = True
print(selectFlag, True)

# final checks
tstampe, tfreqs = sineModelAnalEnhanced(inputFile= '../sms-tools/sounds/sines-440-602-transient.wav')
